...
- If `backendType` is provided:
  ...
  - Calls `terraformModuleGenerator(tree, {...})` with:
    ...
    - Backend type is automatically derived from backend project metadata
- If `backendType` is not provided:
  - Calls `terraformModuleGenerator(tree, {...})` with:
    - `name: 'terraform-infra'` (hardcoded in preset)
...
// 2. Create backend project (if backendType provided)
...
// 3. Create module (stateful if backend exists, simple otherwise)
